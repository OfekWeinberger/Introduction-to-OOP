...
public class Condition {
    ...
    private static final String VARIABLE_TYPE_EXCEPTION = "Variable in condition should be assigned boolean,int or double";
        ...
        if (!s.equals(T) && !s.equals(F)) {
            if (!RegularExpressions.DOUBLE_PATTERN.matcher(s).matches() &&
                ...
                if (subCondStrings.length == 1) {
                    ...
                    if (!(scope.isVarAssigned(Type.BOOLEAN, subCondStrings[0])) && !(scope.isVarAssigned(Type.INT,
                            subCondStrings[0])) && !(scope.isVarAssigned(Type.DOUBLE, subCondStrings[0]))) {
