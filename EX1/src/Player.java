...
public class Player {
	...
	private Move produceRandomMove(Board board) {
		int row, left, right;
		do {
			row = rnd.nextInt(board.getNumberOfRows()) + 1;
		} while (isRowFull(board, row));
		do {
			left = rnd.nextInt(board.getRowLength(row)) + 1;
		} while (!board.isStickUnmarked(row, left));
		do {
			right = rnd.nextInt(board.getRowLength(row) - left + 1) + left;
		} while (!isRandomMoveLegal(board, row, left, right));
		return new Move(row, left, right);
	}

	/*
	 * this method checks if the randomly created move is legal
	 */
	private boolean isRandomMoveLegal(Board board, int row, int left, int right) {
	...
	/*
	 * this method checks if given row is full in the given board
	 */
	private boolean isRowFull(Board board, int row) {
	...
	/*
	 * Produce some intelligent strategy to produce a move: do the best to keep
	 * number of sticks even, so the opponent makes the last move and thus loses the game.
	 */
	private Move produceSmartMove(Board board) {
		/* the number of sticks is odd, so we just want to find a
		 * place where we can mark two sticks, and keep the total number even.*/
		if (board.getNumberOfMarkedSticks() % 2 == 0)
		...
		/* if the number of sticks is even, mark one stick to make it odd,
		 * it also deals with the case of no pair of sticks to mark when the board
		 * has odd number of sticks. */
		for (int row = 1; row <= board.getNumberOfRows(); row++)
			for (int stick = 1; stick <= board.getRowLength(row); stick++)
